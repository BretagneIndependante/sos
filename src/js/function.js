...
async function createTicket(query, files) {
  ...
  if (!error) {
    ...
    const createTicketSQL =
      "INSERT INTO ticket (uuid, description, time_stamp, location) VALUES (?, ?, ?, ?)";
    ...
    for (const file of files) {
      ...
      //insert attachement
      const createAttachementSQL =
        "INSERT INTO piecejointe (uuid, ticket_uuid, link) VALUES (?, ?, ?)";
      const createAttachementParams = [
        attachementUuid,
        ticketUuid,
        newPath.substring(1),
        new Date().getTime(),
      ];
      const createAttachement = await executeQuery(
        createAttachementSQL,
        createAttachementParams
      ).catch((error) => {
        return {
          status: 400,
          message: error,
...
async function sendChat(query) {
  ...
  if (!error) {
    ...
    const getTicket = await executeQuery(getTicketSQL, getTicketParams).catch(
      (error) => {
        return {
          status: 400,
          message: error,
    ...
    if (query.token) {
      ...
      if (getUser.length > 0) {
        ...
        if (getTicket[0].status == "waiting") {
          ...
          const updateTicket = await executeQuery(
            updateTicketSQL,
            updateTicketParams
          );
        }
    ...
    const createChatSQL =
      "INSERT INTO chat (uuid, content, time_stamp, ticket_uuid, is_operator) VALUES (?, ?, ?, ?, ?)";
    ...
    const createChat = await executeQuery(
      createChatSQL,
      createChatParams
    ).catch((error) => {
      return {
  ...
  if (!error) {
    ...
    for (const message of getChat) {
      let messageObject = {
        ...
        is_sender:
          (message.is_operator && isOperator) ||
          (!message.is_operator && !isOperator),
      };
